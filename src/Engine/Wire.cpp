...
void Wire::disconnectSource() { m_source.disconnect(); m_state = PinState::DISCONNECTED; }
void Wire::disconnectDest() { m_dest.disconnect(); }
...
glm::vec4 Wire::getColorFromState() const {
    ...
    return glm::vec4(0.0f, 0.0f, 1.0f, 1.0f);     // Blue (Disconnected/Floating)
}
...
std::vector<float> Wire::getBatchedVertexData() const {
    ...
    // For GL_LINES, we need to output pairs of vertices for each segment.
    // If path is A -> B -> C, we output (A,B), then (B,C).
